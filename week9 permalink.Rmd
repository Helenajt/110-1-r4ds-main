...
```
  ...
  * declare-then-add a method to create a vector.
...
```
  ...
  * human would say no, it is not true.(false)
...
household_income_fct_levelsSetup_ordered <-
  factor(
    ...
    ordered = T
#Is levels ordered? (from the smallest to the larges)
    )
```
